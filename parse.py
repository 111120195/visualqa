...
class DataParser(object):
	...
	def parse(self):
		...
		self.train_dataset = pd.merge(questionId_question_train_df, questionId_answer_train_df,
									  on=['image_id', 'question_id']).drop(['question_id'], axis=1)
		self.val_dataset = pd.merge(questionId_question_val_df, questionId_answer_val_df,
									on=['image_id', 'question_id']).drop(['question_id'], axis=1)
	...
	def save_result(self):
		self.train_dataset.to_csv(self.config.train_data_file)
		self.val_dataset.to_csv(self.config.val_data_file)
